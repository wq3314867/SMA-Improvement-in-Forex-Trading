...
data<-read.csv("INR_USD.csv",header=T)
...
write.table(T1,"INRP3.txt",row.names=FALSE)
...
write.table(T2,"INRMA.txt",row.names=FALSE)
...
write.table(T3,"INRL3.txt",row.names=FALSE)
...
indexL3<-read.table("INRL3pair.txt")
...
indexP3<-read.table("INRP3pair.txt")
...
indexMA<-read.table("INRMApair.txt")
...
par(mfrow=c(2,2))
plot(cumretL3,xlab="INR Cummulative Return By Logrithm",col="blue",type="l",lwd=3)
plot(cumretMA,xlab="INR Cummulative Return SMA",col="green",type="l",lwd=3)
plot(cumretP3,xlab="INR Cummulative Return Powered MA",col="red",type="l",lwd=3)
...
cumretR<-c(max(cumretP3)/max(cumretL3),max(cumretP3)/max(cumretMA),max(cumretL3)/max(cumretMA))
