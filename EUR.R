...
data<-read.csv("EUR_USD.csv",header=T)
...
write.table(T1,"EURP3.txt",row.names=FALSE)
...
write.table(T2,"EURMA.txt",row.names=FALSE)
...
write.table(T3,"EURL3.txt",row.names=FALSE)

#iteration by python:

#by using python to iterate the data, we can get the fllowing results:

indexL3<-read.table("EURL3pair.txt")
...
indexP3<-read.table("EURP3pair.txt")
...
indexMA<-read.table("EURMApair.txt")
...
par(mfrow=c(2,2))
plot(cumretL3,xlab="EUR Cummulative Return By Logrithm",col="red",type="l",lwd=3)
plot(cumretMA,xlab="EUR Cummulative Return SMA",col="green",type="l",lwd=3)
plot(cumretP3,xlab="EUR Cummulative Return Powered MA",col="blue",type="l",lwd=3)
